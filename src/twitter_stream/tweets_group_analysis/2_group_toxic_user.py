from pathes import USER_TOXIC_COUNTS_PATH, GROUPED_USER_TABLE, TOXIC_LABEL, USE_YEARS
import utils
import os
import json
import pandas as pd
...
def main(args):
  ...
  grouped_users_df = pd.DataFrame()
  # create df that column is years and index is user_id
  for toxic in args.toxic_label + ['all']:
    yearly_users_df[toxic] = pd.DataFrame(columns=args.years + ['group'])
